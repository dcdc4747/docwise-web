...
文档翻译智能体——**Agent 是大脑，程序是手脚**。用户上传英文文献 PDF，获得"像原文档的中文版"：翻译、排版、质检一条龙。排版基于**两个成熟开源引擎（pdf2zh / BabelDOC）编排成「快 / 中 / 慢」三档**，再叠加质检闭环与术语一致性。
...
> 战略转向（详见仓库 `AGENTS.md` 项目记忆）：**不再自研排版引擎**，改为**编排 pdf2zh / BabelDOC + 中间层魔改（质检 / 术语 / 打回 / DOM）**，形成「快 / 中 / 慢」三档。

| 阶段 | 内容 | 状态 |
| --- | --- | --- |
| 0 | 项目骨架（后端/前端/数据库可启动） | ✅ 已完成（2026-08-06 验收通过） |
| 1 | 最小翻译链路：接 pdf2zh 快档跑通"取字→翻译→出稿"，保证每块状态可见、不静默截断 | 待开工 |
| 2 | Web 端闭环（MVP）：上传/进度/预览/下载/历史 + 档位选择 + BabelDOC 中档；SSE+轮询 | 待开工 |
| 3 | 中间表示 DOM（v1 标题/段落/图片）+ 引擎编排强化，吸收 BabelDOC"先抽结构再重建" | 待开工 |
| 4 | 轻量角色化编排 + 精档质检打回闭环（翻译/排版决策/审校 + Qwen 交叉验证） | 待开工 |
| 5 | 知识库与记忆：术语表/用户偏好/跨章节一致 + 对接 BabelDOC 自动术语抽取 | 待开工 |
| 6 | 扫描件/图片通道：BabelDOC OCR 兜底 + 视觉三方分工 | 待开工 |
| 7 | 质量评估与回归（固定测试集约 20 篇） | 待开工 |
| 8 | 竞赛材料（演示/PPT/文档/答辩） | 待开工 |
